...
class Nav extends Widget
{
     ...
     * @since 2.0.7
     */
    public $dropdownClass = 'common\themes\lightbootstrap\widgets\Dropdown';
    ...
    public function renderItem($item)
    {
        ...
        if (isset($item['icon'])) {
            $icon = $encodeLabel ? Html::encode($item['icon']) : $item['icon'];
            $label = $icon . $label;
        }
     ...
     * @since 2.0.1
     */
    protected function renderDropdown($items, $parentItem, $parentActive)
    {
        /** @var Widget $dropdownClass */
        $dropdownClass = $this->dropdownClass;
        $url = str_replace('#','',ArrayHelper::getValue($parentItem, 'url', '#'));
        $class = $parentActive?'collapse in':'collapse';
        return 
        Html::beginTag('div',['class'=>$class, 'id'=>$url]).
